#!/usr/bin/env bash
: '
Download cartoon images from an inaccessible xkcd directory.

Usage:  ./scrape_imgs.sh [path_local_directory] 
...
filenames are known; hence, the filenames would first need to be discovered (i.e. scraped from index.html files).

This script fetches index.html files by looping over [i] and detecting URLs in the HTML files which point
