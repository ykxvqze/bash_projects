...
__print_usage() {
    ...
    __battery_status           # Show battery percentage charge remaining
    __userinfo                 # List users currently logged in
    __ports_open               # List TCP ports open on localhost
    __sysinfo                  # List user/superuser, OS info, RAM, WAN/LAN/gateway IP addresses
    ...
    __debugmode [ -s | -u ]    # Set an informative PS4 (debug) prompt and enable xtrace via -s;
