...
ARGS:
        filename(s): Word document(s) with filename format: *.docx
                     Filenames may include paths, e.g. /dir/subdir/file1.docx
                     Filenames should not begin with a hyphen (-)

OUTPUT:
        files (*_formatted.docx) with metadata removed in the same directory
        as the original (input) files.
...
document.xml and comments.xml (if the latter is present). Metadata in the
